...
def execute_single(single_instance):
    ...
    # assume we are running emulation
        ...
        area_upper_bound = single_instance['area_upper_bound']
        resize_type = single_instance['resize_type']
        fixed_area = single_instance['fixed_area']
        fixed_ratio = single_instance['fixed_ratio']
        track_region = single_instance['track_region']
        if resize_type == 'fixed_ratio':
            target_type = 'ratio'
            target_val = fixed_ratio
            if fixed_ratio == 1:
                area_upper_bound = 0.0
        elif resize_type == 'fixed_area':
            target_type = 'area'
            target_val = fixed_area
        ...
        result_file_name = (f'{video_name}_streamBpack_{context_padding_type}_{context_val}_'
                            f'{blank_padding_type}_{blank_val}_min_{area_upper_bound}_'
                            f'{target_type}_{target_val}_inter_{intersect_iou}_merge_{merge_iou}_'
                            f'track_{track_region}')
        if single_instance['overwrite'] == False and \
