...
def calc_iou(a, b):
    ...
    if union_area == 0:
        print(f"a area {calc_area(a)}, b area {calc_area(b)}, inter area {intersection_area}")
        return 1
...
def write_list_txt(fname, wlist):
    list_file = open(fname, "w")
    for list_item in wlist:
        str_to_write = (f"{list_item},")
        list_file.write(str_to_write)
    list_file.close()

def write_list_csv(fname, wlist):
    list_file = open(fname, "w")
    csv_writer = csv.writer(list_file)
    for list_item in wlist:
        row = [list_item]
        csv_writer.writerow(row)
    list_file.close()

def write_list(fname, wlist):
    if re.match(r"\w+[.]csv\Z", fname):
        write_list_csv(fname, wlist)
    else:
        write_list_txt(fname, wlist)
